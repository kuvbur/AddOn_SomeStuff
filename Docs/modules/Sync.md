...
|---------|-------------|------------|
| `MonAll` | 64 | Мониторинг для всех активных типов [из комментария]; вызывается из SomeStuff_Main.cpp:400/555 [по коду, grep] |
...
- Вызывается из: `MenuCommandHandler` (SomeStuff_Main.cpp:405). [по коду, grep]
