...
3. Копируем скачанный файл в папку с расширениями Архикада (например, _C:\Program Files\GRAPHISOFT\ARCHICAD 24\Расширения ARCHICAD\Дополнения_)
4. Запускаем архикад.
...
2. После скачивания и распаковки архива :
   ...
   - Снимите атрибуты карантина:
     ...
     ```
     _(При необходимости используйте `sudo xattr -cr SomeStuff.bundle`)_
   - **Подпишите аддон** (обязательный шаг для новых версий macOS):
     ```bash
     codesign --force --deep --sign - SomeStuff.bundle
     ```
3. Запускаем архикад.
4. В менеджере дополнений нажимаем Добавить и указываем на аддон.
