...
1.  **Базовый элемент**  
    ...
    - _v1.74+:_ При наличии `ИМЯ_ИЗБРАННОГО` в правиле используется Избранный элемент
2.  **Добавьте свойство**  
...
3.  **Формат описания:**  
    ...
    - `g(...)` **Группа исходных данных**:
      - `U1,U2,U3`: Параметры группировки (уникальные комбинации) _v1.78+:_ Если поставить прочерк - то в качестве уникальных будут использованы параметры для передачи
...
- **Запись GUID исходных элементов**  
  ...
  _Необходимо для подсветки исходных (родительских) элементов командой "Показать связанные элементы"_
- **Запись имени правила**  
  Добавьте свойство: `Spec_rule_name`  
  _Позволяет фильтровать созданные элементы в Интерактивном Каталоге_
