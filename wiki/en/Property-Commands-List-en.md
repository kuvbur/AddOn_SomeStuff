...
| Command             | Description                                                                                                                            |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `Sync_flag`         | A flag property to disable element synchronization.<br>**Type:** Criteria. Can be calculated by a formula.                             |
| `Sync_class_flag`   | Flag to enable classification processing (_v1.72+_).<br>If not found, `Sync_flag` is used. Other functions remain operational.         |
| `Sync_correct_flag` | Flag to disable coordinate accuracy verification (_v1.72+_).<br>If not found, `Sync_flag` is used. Other functions remain operational. |

## Synchronization within Properties of a Single Object

`Sync_from` - reads values FROM another location and writes TO the current property. `Sync_to` - writes the value of the current property to another location (property, object parameter, project info).
A property description can contain multiple `Sync_from` commands; the first value found will be written.

| Command                                                        | Description                                                                          |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `Sync_from`                                                    | Reads values from a source and writes them to the current property                   |
| `Sync_to`                                                      | Writes the value of the current property to the target location                      |
| **Reading Data (`Sync_from`)**                                 |                                                                                      |
| `Sync_from{PARAMETER_NAME}`                                    | Copies the value of a GDL parameter to the property (parameter name in Latin script) |
...
| `Sync_to{PARAMETER_NAME}`                                      | Writes the property value to a GDL parameter of the element (_v1.6+_)                |
...
### Writing to Project Information _v1.6+_
...
- `Sync_to{Attribute:Layer}` - the search is performed by the full layer name (with extension)

### Retrieving Element Information _v1.77+_

- `Sync_from{Element:Material overridden}` - Is material override enabled or disabled?
- `Sync_from{Element:Zone manual poly}` - Is the Zone created manually, with all edges static? _v1.78_
- `Sync_from{Element:zone by ref line}` - Is the Zone surrounded by the reference line of Walls, not the side lines? _v1.78_

### Retrieving MEP Element Information (AC28+) _v1.77+_

Processes pipes, ducts, transitions, bends, and routes.
...
| `Sync_from{MEP:element set name}` | Name of the element table (pipe/bend/transition) |
...
- `Renum_flag{property name with rule ; SPACE}` _v1.6+_
...
- `Renum{criterion property name; grouping property name}`
