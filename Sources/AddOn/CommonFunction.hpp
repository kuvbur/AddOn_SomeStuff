...
// -----------------------------------------------------------------------------
// Отладочная диагностика (TESTING): открыт ли тестовый проект (test_25 и т.п.).
// Используется DBprnt/DBtest для записи в test_results.txt и логированием
// пути файла настроек (#190) — чтобы строка попадала в лог только когда
// проект уже открыт и вывод виден.
// -----------------------------------------------------------------------------
...
UInt32 StringSplt (const GS::UniString &instring,
                   ...
                   bool filter_empty,
                   GS::Array<GS::UniString> *scratch_parts = nullptr);
