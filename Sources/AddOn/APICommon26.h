...
template <typename... Args> void WriteReport (const char *format, Args &&...args) {
    ACAPI_WriteReport (format, false, std::forward<Args> (args)...);
}

template <typename... Args> void WriteReport_Alert (const char *format, Args &&...args) {
...
bool ClickAPoint (const char *prompt, API_Coord *c);

bool GetAnArc (
    const char *prompt, API_Coord *origin, API_Coord *startPos, API_Coord *endPos, bool *isArcNegative = nullptr);
...
GS::Array<API_Neig> ClickElements_Neig (const char *prompt, API_ElemTypeID needTypeID);

GS::Array<API_Guid> ClickElements_Guid (const char *prompt, API_ElemTypeID needTypeID);
