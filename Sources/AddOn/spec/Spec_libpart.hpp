...
// Подсистема разбора и хранения данных спецификаций/списков для библиотечных элементов.
namespace ListData {
    ...
    // -----------------------------------------------------------------------------
    // Возвращает массив всех пар "ключ-значение" параметров библиотечного элемента.
    // -----------------------------------------------------------------------------
    GS::Array<GS::Pair<GS::UniString, GS::UniString>> GetAllKeys (const LibElement &el);

    // -----------------------------------------------------------------------------
    // Добавляет общий материал или элемент в сметную структуру.
    // -----------------------------------------------------------------------------
    void Add (LibElement &paramListDataToRead, GS::UniString &name, GS::UniString &unitcode, double &qty);

    // -----------------------------------------------------------------------------
    // Записывает распарсенные сметные данные в словарь параметров элемента.
    // -----------------------------------------------------------------------------
