...
namespace AutoFunc {
    ...
    // Возвращает индекс inx элемента в массиве, если точка была концом отрезка -
    // поднимает флаг isend
    // -----------------------------------------------------------------------------
    ...
    GetSectLine (API_Guid &elemguid, GS::Array<SSectLine> &lines, GS::UniString &id, const Point2D &startpos) {
        ...
        if (tostart.GetLength () > toend.GetLength ())
            start = end;
        bool isend = false;
        UInt32 n = k.GetSize ();
    ...
    // -----------------------------------------------------------------------------
    GSErrCode DoSect (SSectLine &sline,
        ...
        if (documentFrom3DType.cutSetting.shapes != nullptr)
            BMKillHandle ((GSHandle *)&(documentFrom3DType.cutSetting.shapes));
        documentFrom3DType.cutSetting.shapes = reinterpret_cast<API_3DCutShapeType **> (
            BMAllocateHandle (cutInfo.nShapes * sizeof (API_3DCutShapeType), ALLOCATE_CLEAR, 0));
        if (documentFrom3DType.cutSetting.shapes != nullptr) {
    ...
    GSErrCode PlaceDocSect (SSectLine &sline, API_Element &elemline) {
        ...
        err = ACAPI_CallUndoableCommand ("Create text", [&] () -> GSErrCode {
            GSErrCode err = NoError;
            if (!elemList.IsEmpty ())
                err = ACAPI_Element_Delete (elemList);
            if (err != NoError) {
    ...
    void ProfileByLine () {
        ...
        err = ACAPI_CallCommand ("Create profile by line", [&] () -> GSErrCode {
            GSErrCode err = NoError;
        ...
        for (UInt32 i = 0; i < lines.GetSize (); i++) {
            if (PlaceDocSect (lines[i], elemline) != NoError) {
                ...
                // err = ACAPI_Database (APIDb_ChangeCurrentDatabaseID,
                // &databasestart, nullptr); if (err != NoError) {
                //    msg_rep ("ProfileByLine", "APIDb_ChangeCurrentDatabaseID",
                //    err, APINULLGuid); return;
                //}
                // err = ACAPI_Automate (APIDo_ChangeWindowID, &windowstart,
                // nullptr); if (err != NoError) {
                //    msg_rep ("ProfileByLine", "APIDo_ChangeWindowID", err,
                //    APINULLGuid); return;
                //}
            }
        }
    ...
    GSErrCode AlignOneDrawingsByPoints (const API_Guid &elemguid,
        ...
        if (GetElemTypeID (element) != API_DrawingID)
        ...
        for (UInt32 i = 0; i < hotspotList.GetSize (); i++) {
            ...
            if (err == NoError) {
                ...
                if (hotspotelem.header.layer == layer &&
                    (hotspotelem.hotspot.pen == 143 || hotspotelem.hotspot.pen == 163)) {
        ...
        for (UInt32 i = 1; i < hotspotcoord.GetSize (); i++) {
            if (hotspotcoord[i].x < leftpos.x)
                leftpos = hotspotcoord[i];
            if (hotspotcoord[i].x > rightpos.x)
                rightpos = hotspotcoord[i];
        }
        double l = fabs (rightpos.x - leftpos.x);
        // Для чертежей с неподвижным началом не будем добавлять длину чертежа для
        // последующих
        if (flag_find) {
