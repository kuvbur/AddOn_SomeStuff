...
public:
	...
	void FormatToMax(RenumPos& pos, short nulltype) {
		...
		if (npos > nthis) {
			...
			char charrepl = ' ';
			if (nulltype == ADDZEROS || nulltype == ADDMAXZEROS) charrepl = '0';
	...
	bool operator== (const RenumPos& b) {
		if (this->isNum == b.isNum && this->isNum) {
			if (this->prefix != b.prefix) return false;
			if (this->suffix != b.suffix) return false;
			if (this->numpos != b.numpos) return false;
		}
		else {
			if (this->strpos != b.strpos) return false;
		}
		return true;
	}
