...
// Возвращение на исходную БД и окно
    ...
    void GetAllChangesMarker (GS::HashTable<GS::UniString, API_Guid> &layout_note_guid) {
            ...
            if (err == NoError) {
                ...
                if (err == NoError) {
...
#else
                    err = ACAPI_Database (
                        APIDb_GetRVMLayoutCurrentRevisionChangesID, &(dbInfo.databaseUnId), &layoutchange);
#endif
                    if (!layoutchange.IsEmpty ()) {
                        ...
                        if (CheckChanges (changes, revision.layoutInfo.subsetName, revision.layoutInfo.id))
                            has_change_error = true;
                        ChangeMarkerTextOnLayout (changes);
                        ChangeLayoutProperty (changes,
                                              layout_note_guid,
                                              dbInfo.databaseUnId,
                                              revision.layoutInfo.id,
                                              layoutRVI,
                                              allchanges);
                    }
                } else {
        ...
        if (has_change_error)
            msg_rep (EMPTYSTRING, "Check Marker reservation", APIERR_GENERAL, APINULLGuid, true);
...
#endif
                if (err != NoError)
                    msg_rep ("GetAllChangesMarker", "APIEnv_ChangeLayoutSetsID", err, APINULLGuid);
            }
    ...
    bool ChangeLayoutProperty (ChangeMarkerDict &changes,
        ...
        for (std::map<std::string, GS::UniString, doj::alphanum_less<std::string>>::iterator k = abc_changes.begin ();
            ...
            if (layout_note_guid.ContainsKey (prop_name)) {
                ...
                if (change.typeizm == TypeIzm)
                    str = str + ATSIGN + izmString;
                if (change.typeizm == TypeZam)
                    str = str + ATSIGN + zamString;
                if (change.typeizm == TypeNov)
                    str = str + ATSIGN + novString;
                if (change.typeizm == TypeAnnul)
                    str = str + ATSIGN + annulString;
                if (!change.fam.IsEmpty ())
                    str = str + ATSIGN + change.fam;
                flag_write = true;
                if (layoutInfo.customData->ContainsKey (prop_guid)) {
                    layoutInfo.customData->Set (prop_guid, str);
                } else {
...
#endif
            if (err != NoError)
                msg_rep ("GetChangesMarker", "APIEnv_ChangeLayoutSetsID", err, APINULLGuid);
        }
        delete layoutInfo.customData;
        return true;
    }
        ...
        for (auto ch : changes) {
            ...
            for (UInt32 i = 0; i < change.arr.GetSize (); i++) {
                if (change.arr[i].markerguid == APINULLGuid) {
                    if (change.arr[i].typeizm != TypeNone)
                        typeizm_layout = change.arr[i].typeizm;
                    if (change.arr[i].fam.GetLength () > 3)
                        fam_layout = change.arr[i].fam;
                } else {
                    hasmarker = true;
                    if (change.arr[i].typeizm == TypeIzm)
                        hasmarkerIzm = true;
                    if (typeizm_marker != TypeNone && change.arr[i].typeizm != TypeNone &&
                        change.arr[i].typeizm != typeizm_marker) {
                        msg_rep ("GetChangesMarker",
                                 "Different type : " + change.arr[i].changeId + " sheet ID " + subsetName + SLASH +
                                     layoutid,
                                 APIERR_GENERAL,
                                 APINULLGuid,
                                 false);
                    }
                    if (change.arr[i].fam.GetLength () > 3 && fam_marker.GetLength () > 3 &&
                        !fam_marker.IsEqual (change.arr[i].fam)) {
                        msg_rep ("GetChangesMarker",
                                 "Different surname : " + fam_marker + "<->" + change.arr[i].fam + " on " +
                                     change.arr[i].changeId + " sheet ID " + subsetName + SLASH + layoutid,
                                 APIERR_GENERAL,
                                 APINULLGuid,
                                 false);
                        has_error = true;
                    ...
                    if (change.arr[i].fam.GetLength () > 3)
                        fam_marker = change.arr[i].fam;
                    if (change.arr[i].typeizm != TypeNone)
                        typeizm_marker = change.arr[i].typeizm;
                    if (ACAPI_Element_Filter (change.arr[i].markerguid, APIFilt_InMyWorkspace)) {
                        if (ACAPI_Element_Filter (change.arr[i].markerguid, APIFilt_HasAccessRight)) {
                            if (!ACAPI_Element_Filter (change.arr[i].markerguid, APIFilt_IsEditable)) {
                                msg_rep ("GetChangesMarker",
                                         "Marker not editable : " + change.arr[i].changeId + " sheet ID " + subsetName +
                                             SLASH + layoutid,
                                         APIERR_GENERAL,
                                         APINULLGuid,
                                         false);
                            ...
                            msg_rep ("GetChangesMarker",
                                     "Marker has not access right : " + change.arr[i].changeId + " sheet ID " +
                                         subsetName + SLASH + layoutid,
                                     APIERR_GENERAL,
                                     APINULLGuid,
                                     false);
                            has_error = true;
                        ...
                        msg_rep ("GetChangesMarker",
                                 "Marker not in on workspace : " + change.arr[i].changeId + " sheet ID " + subsetName +
                                     SLASH + layoutid,
                                 APIERR_GENERAL,
                                 APINULLGuid,
                                 false);
            ...
            if (fam_marker.GetLength () > 3)
                fam_layout = fam_marker;
            if (hasmarker && typeizm_marker != TypeNone)
                typeizm = typeizm_marker;
            if (!hasmarker)
                typeizm = typeizm_layout;
            if (hasmarkerIzm && hasmarker)
                typeizm = TypeIzm;
            if (typeizm == TypeNone && hasmarker)
                typeizm = TypeIzm;
            ...
            for (UInt32 i = 0; i < change.arr.GetSize (); i++) {
                ...
                if (change.changeId.IsEmpty ())
                    change.changeId = change.arr[i].changeId;
                if (change.nizm.IsEmpty ())
                    change.nizm = change.arr[i].nizm;
                if (!note.IsEmpty () && !change.arr[i].note.IsEmpty ())
                    note = note + " ;@ " + change.arr[i].note;
                if (note.IsEmpty () && !change.arr[i].note.IsEmpty ())
                    note = change.arr[i].note;
                if (change.code == 0 && change.arr[i].code > 0)
                    change.code = change.arr[i].code;
            }
    ...
    void GetChangesLayout (GS::Array<API_RVMChange> &layoutchange,
        ...
        for (auto c : layoutchange) {
            ...
            if (n > 3) {
                if (ch.typeizm == TypeNone && partstring[2].Contains (izmString))
                    ch.typeizm = TypeIzm;
                if (ch.typeizm == TypeNone && partstring[2].Contains (zamString))
                    ch.typeizm = TypeZam;
                if (ch.typeizm == TypeNone && partstring[2].Contains (novString))
                    ch.typeizm = TypeNov;
                if (ch.typeizm == TypeNone && partstring[2].Contains (annulString))
                    ch.typeizm = TypeAnnul;
    ...
    bool GetChangesMarker (ChangeMarkerDict &changes) {
        ...
        GS::UniString annulString =
            RSGetIndString (ID_ADDON_STRINGS + isEng_, Annul_StringID, ACAPI_GetOwnResModule ());
        GSErrCode err = NoError;
        ...
        for (UInt32 i = 0; i < guidArray.GetSize (); i++) {
            ...
            if (err == NoError && element.header.hasMemo) {
                ...
                GS::Int32 typeizm = TypeNone;
                GS::Int32 code = 0;
                if (GetMarkerPos (guidArray[i], startpoint) &&
                    GetMarkerText (element.changeMarker.markerGuid, note, nuch, nizm, typeizm, fam, code)) {
                    Change ch;
