...
static const GS::UniString MAT_BUILDING_MATERIAL_DESCRIPTION = "@property:BuildingMaterialProperties/Building Material Description";
static const GS::UniString MAT_BUILDING_MATERIAL_DENSITY = "@property:BuildingMaterialProperties/Building Material Density";
static const GS::UniString MAT_BUILDING_MATERIAL_MANUFACTURER = "@property:BuildingMaterialProperties/Building Material Manufacturer";
static const GS::UniString MAT_BUILDING_MATERIAL_CUTFILL = "@property:BuildingMaterialProperties/Building Material CutFill";
...
GS::UniString NumToString (double var, const FormatString& stringformat)
{
    ...
    if (sep_idx != -1) buf[sep_idx] = !stringformat.delimetr.IsEmpty () ? (char) stringformat.delimetr.GetChar (0) : '.';
...
bool GetRuleFromSelected (const API_Guid& elemguid, GS::HashTable<API_Guid, API_PropertyDefinition>& definitions, const GS::UniString& name, bool check_bracket)
{
    ...
    for (const auto& definition : definitions_) {
        ...
        if (!definition.description.Contains (name)) continue;
        if (check_bracket) {
            if (!definition.description.Contains (BRACESTART)) {
                msg_rep ("GetRuleFromSelected", definition.name + " Check the opening bracket, there should be {", APIERR_GENERAL, APINULLGuid);
                continue;
            }
            if (!definition.description.Contains (BRACEEND)) {
                msg_rep ("GetRuleFromSelected", definition.name + " Check the closing bracket, there should be }", APIERR_GENERAL, APINULLGuid);
                continue;
            }
        }
        if (definitions.ContainsKey (definition.guid)) continue;
...
void GetRelationsElement (const API_Guid& elemGuid, const  API_ElemTypeID& elementType, const SyncSettings& syncSettings, GS::Array<API_Guid>& subelemGuid, bool addZone, bool addConnect)
{
    ...
    #ifndef AC_22
    auto GetHierarchicalOwners = [&](API_Guid& owner, API_Guid& ownerRoot, API_HierarchicalElemType& type, API_HierarchicalElemType& typeRoot) {
    ...
    switch (elementType) {
        ...
        case API_WallID:
            if (syncSettings.widoS && addConnect) {
                ...
                #if defined(AC_27) || defined(AC_28) || defined(AC_29)
                err = ACAPI_Grouping_GetConnectedElements (elemGuid, API_WindowID, &connectedElements, APIFilt_IsEditable | APIFilt_HasAccessRight | APIFilt_InMyWorkspace, APINULLGuid);
                #else
                ...
                #if defined(AC_27) || defined(AC_28) || defined(AC_29)
                err = ACAPI_Grouping_GetConnectedElements (elemGuid, API_DoorID, &connectedElements, APIFilt_IsEditable | APIFilt_HasAccessRight | APIFilt_InMyWorkspace, APINULLGuid);
                #else
        ...
        case API_RailingID:
            if (syncSettings.cwallS) err = GetRElementsForRailing (elemGuid, subelemGuid);
            break;
        case API_CurtainWallID:
            if (syncSettings.cwallS) err = GetRElementsForCWall (elemGuid, subelemGuid);
            break;
        ...
        case API_CurtainWallPanelID:
            if (syncSettings.cwallS) {
                ...
                if (owner != APINULLGuid && type == API_ChildElemInMultipleElem) subelemGuid.Push (owner);
                if (ownerRoot != owner && ownerRoot != APINULLGuid && typeRoot == API_ChildElemInMultipleElem) subelemGuid.Push (ownerRoot);
                #endif
            }
            break;
        case API_DoorID:
        ...
        case API_ZoneID:
            if (syncSettings.objS && addZone) {
                ...
                if (err == NoError) {
                    ...
                    #else
                    static const API_ElemTypeID baseTypes[] = { API_ObjectID, API_LampID, API_StairID, API_RiserID, API_TreadID, API_StairStructureID };
                    for (const auto& type : baseTypes) {
                        if (const auto* arrayPtr = relData.elementsGroupedByType.GetPtr (type)) {
                            for (const auto& id : *arrayPtr) subelemGuid.Push (id);
                        }
                    }
                    if (syncSettings.wallS) {
                        static const API_ElemTypeID wallTypes[] = { API_WallID, API_SlabID, API_ColumnID, API_BeamID, API_RoofID, API_ShellID, API_MorphID };
...
short ParamHelpers::GetTypeInxByRawnamePrefix (const GS::UniString& rawName)
{
    ...
    if (rawName.BeginsWith (FILENAMEPREFIX)) return FILETYPEINX;
    #if defined(TESTING)
...
void ParamHelpers::SetParamValueSourseByName (ParamValue& pvalue)
{
    ...
    if (pvalue.fromGDLparam || pvalue.fromGDLdescription || pvalue.fromListData) ParamHelpers::SetArrayByRawname (pvalue);
...
{
    if (name.IsEmpty ()) return EMPTYSTRING;
    GS::UniString rawname_prefix = "";
    GS::UniString name_ = name.ToLowerCase ();
    if (name_.Contains (BRACESTART) && name_.Contains (BRACESTART)) name_ = name_.GetSubstring (CHARBRACESTART, CHARBRACEEND, 0);
...
void ParamHelpers::AddParamDictValue2ParamDictElement (const API_Guid& elemGuid, ParamDictValue& param, ParamDictElement& paramToRead)
{
    ...
    for (auto cIt = param.EnumeratePairs (); cIt != nullptr; ++cIt) {
        ...
        const GS::UniString& rawName = *cIt->key;
        #endif
        if (pPtr->ContainsKey (rawName)) continue;
        #if defined(AC_28) || defined(AC_29)
...
bool ParamHelpers::AddProperty (ParamDictValue& params, GS::Array<API_Property>& properties, const API_Guid& elemguid)
{
    ...
    for (API_Property& property : properties) {
        ...
        if (hasname) {
            if (pvalue.fromClassification && !pvalue.val.uniStringValue.IsEmpty () && ClassificationFunc::ReadSystemDict ()) {
                ...
                if (classguid != APINULLGuid && pvalue.val.guidval == APINULLGuid) {
                    ...
                    #if defined(TESTING)
                    if (pvalue.val.guidval != APINULLGuid) DBprnt ("ERROR Compare classification - double class");
                    if (classguid == APINULLGuid) DBprnt ("ERROR Compare classification - empty class");
                    #endif
                }
            }
            ...
            if (needAdd) {
                if (elemguid != APINULLGuid) pvalue.fromGuid = elemguid;
                params.Put (rawName, std::move (pvalue));
                flag_find = true;
            }
        }
...
bool ParamHelpers::ReadCoords (const API_Element& element, ParamDictValue& pdictvaluecoord)
{
    ...
    if (GetParamValueFromCache (locorig, cacheparam)) lox = cacheparam.val.rawDoubleValue;
    locorig = "{@glob:locorigin_y}";
    if (GetParamValueFromCache (locorig, cacheparam)) loy = cacheparam.val.rawDoubleValue;
    locorig = "{@glob:locorigin_z}";
    if (GetParamValueFromCache (locorig, cacheparam)) loz = cacheparam.val.rawDoubleValue;
    locorig = "{@glob:offsetorigin_x}";
    if (GetParamValueFromCache (locorig, cacheparam)) offx = cacheparam.val.rawDoubleValue;
...
void CoordNorthAngle (double north, double angz, double& angznorth, GS::UniString& angznorthtxt, GS::UniString& angznorthtxteng)
{
    ...
    double nn = 360.0;
    const Int32 iseng = ID_ADDON_STRINGS + isEng ();
    if (angznorth > nn - 22.5 || angznorth < n + 22.5) angznorthtxt = RSGetIndString (iseng, N_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > ne - 22.5 && angznorth < ne + 22.5) angznorthtxt = RSGetIndString (iseng, NE_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > e - 22.5 && angznorth < e + 22.5) angznorthtxt = RSGetIndString (iseng, E_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > se - 22.5 && angznorth < se + 22.5) angznorthtxt = RSGetIndString (iseng, SE_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > s - 22.5 && angznorth < s + 22.5) angznorthtxt = RSGetIndString (iseng, S_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > sw - 22.5 && angznorth < sw + 22.5) angznorthtxt = RSGetIndString (iseng, SW_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > w - 22.5 && angznorth < w + 22.5) angznorthtxt = RSGetIndString (iseng, W_StringID, ACAPI_GetOwnResModule ());
    if (angznorth > nw - 22.5 && angznorth < nw + 22.5) angznorthtxt = RSGetIndString (iseng, NW_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, n + 22.5)) angznorthtxt = RSGetIndString (iseng, N_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, ne + 22.5)) angznorthtxt = RSGetIndString (iseng, NE_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, e + 22.5)) angznorthtxt = RSGetIndString (iseng, E_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, se + 22.5)) angznorthtxt = RSGetIndString (iseng, SE_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, s + 22.5)) angznorthtxt = RSGetIndString (iseng, S_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, sw + 22.5)) angznorthtxt = RSGetIndString (iseng, SW_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, w + 22.5)) angznorthtxt = RSGetIndString (iseng, W_StringID, ACAPI_GetOwnResModule ());
    if (is_equal (angznorth, nn - 22.5)) angznorthtxt = RSGetIndString (iseng, NW_StringID, ACAPI_GetOwnResModule ());
...
{
    auto& cache = PROPERTYCACHE ();
    if (!name.Contains (PROP_PREFIX)) return name;
    if (name.Contains (PROP_SYNC_NAME)) return name;
    if (name.IsEqual (PROP_ID))                    return MAT_BUILDING_MATERIAL_ID;
    if (name.IsEqual (PROP_N))                     return MAT_N;
    if (name.IsEqual (PROP_NS))                    return MAT_NS;
    if (name.IsEqual (PROP_LAYER_THICKNESS))       return MAT_LAYER_THICKNESS;
    if (name.IsEqual (PROP_TH))                    return MAT_LAYER_THICKNESS;
    if (name.IsEqual (PROP_LAYER_MIN_THICKNESS))   return MAT_LAYER_MIN_THICKNESS;
    if (name.IsEqual (PROP_TH_MIN))                return MAT_LAYER_MIN_THICKNESS;
    if (name.IsEqual (PROP_BMAT_INX))              return MAT_BMAT_INX;
    if (name.IsEqual (PROP_CUTFILL_INX))           return MAT_CUTFILL_INX;
    if (name.IsEqual (PROP_SOME_STUFF_TH))         return MAT_SOME_STUFF_TH;
    if (name.IsEqual (PROP_SOME_STUFF_UNITS))      return MAT_SOME_STUFF_UNITS;
    if (name.IsEqual (PROP_UNIT))                  return MAT_SOME_STUFF_UNITS;
    if (name.IsEqual (PROP_KZAP))                  return MAT_SOME_STUFF_KZAP;
    if (name.IsEqual (PROP_AREA))                  return MAT_AREA;
    if (name.IsEqual (PROP_VOLUME))                return MAT_VOLUME;
    if (name.IsEqual (PROP_QTY))                   return MAT_QTY;
    if (name.IsEqual (PROP_UNIT_PREFIX))           return MAT_UNIT_PREFIX;
    if (name.IsEqual (PROP_LENGTH))                return MAT_LENGTH;
    if (name.IsEqual (PROP_AREA_SECTION))          return MAT_AREA_SECTION;
    if (name.IsEqual (PROP_WIDTH))                 return MAT_WIDTH;
    if (name.IsEqual (cache.buildingMaterialNameString)) return MAT_BUILDING_MATERIAL_NAME;
    if (name.IsEqual (cache.buildingMaterialDescriptionString)) return MAT_BUILDING_MATERIAL_DESCRIPTION;
    if (name.IsEqual (cache.buildingMaterialDensityString)) return MAT_BUILDING_MATERIAL_DENSITY;
    if (name.IsEqual (cache.buildingMaterialManufacturerString)) return MAT_BUILDING_MATERIAL_MANUFACTURER;
    if (name.IsEqual (cache.buildingMaterialCutFillString)) return MAT_BUILDING_MATERIAL_CUTFILL;
    if (name.IsEqual (cache.thicknessString)) return MAT_LAYER_THICKNESS;
    return name;
}
...
bool ParamHelpers::ParseParamName (GS::UniString& expression, ParamDictValue& paramDict)
{
    ...
    if (expression.FindFirst (CHARBRACESTART) == MaxUIndex) return false;
    GS::UniString result;
    ...
    while (lastPos < len) {
        ...
        if (startBrace > lastPos) result.Append (expression.GetSubstring (lastPos, startBrace - lastPos));
        ...
        if (!paramDict.ContainsKey (part_)) {
            ...
            switch (inx) {
                case PROPERTYTYPEINX:     pvalue.fromPropertyDefinition = true; break;
                case GDLTYPEINX:          pvalue.fromGDLparam = true; break;
                case GDLDESCTYPEINX:      pvalue.fromGDLparam = true; pvalue.fromGDLdescription = true; break;
                case COORDTYPEINX:        pvalue.fromCoord = true; break;
                case GLOBTYPEINX:         pvalue.fromGlob = true; break;
                case INFOTYPEINX:         pvalue.fromInfo = true; break;
                case MEPTYPEINX:          pvalue.fromMEP = true; break;
                case FORMULATYPEINX:      pvalue.val.hasFormula = true; break;
                case IDTYPEINX:           pvalue.fromID = true; break;
                case IFCTYPEINX:          pvalue.fromIFCProperty = true; break;
                case MORPHTYPEINX:        pvalue.fromMorph = true; break;
                case ATTRIBTYPEINX:       pvalue.fromAttribElement = true; break;
                case LISTDATATYPEINX:     pvalue.fromListData = true; break;
                case MATERIALTYPEINX:     pvalue.fromMaterial = true; break;
                case CLASSTYPEINX:        pvalue.fromClassification = true; break;
                case ELEMENTTYPEINX:      pvalue.fromElement = true; break;
                case FILETYPEINX:         pvalue.fromFile = true; break;
                default: break;
            }
        ...
        if (!formatstring.isEmpty) part_.ReplaceAll (BRACEEND, DOT + formatstring.stringformat + BRACEEND);
        result.Append (part_);
        lastPos = endBrace + 1;
    }
    expression = std::move (result);
...
{
    if (pdictvalue.IsEmpty () || expression.IsEmpty ()) return false;
    if (expression.FindFirst (CHARBRACESTART) == MaxUIndex) return true;
    ...
    while (expression.FindFirst (CHARBRACESTART) != MaxUIndex && iteration < MAX_ITERATIONS) {
        ...
        while (lastPos < len) {
            ...
            if (!part.IsEmpty ()) {
                ...
                GS::UniString partc_current = !attribsuffix.IsEmpty () ? (BRACESTART + part_clean + attribsuffix) : GS::UniString ();
                GS::UniString partc_old = !attribsuffix_old.IsEmpty () ? (BRACESTART + part_clean + attribsuffix_old) : GS::UniString ();
                ...
                if (pvaluePtr != nullptr && pvaluePtr->isValid) {
                    ...
                    #if defined(TESTING)
                    if (pvaluePtr != nullptr && !pvaluePtr->isValid) {
                        DBprnt ("ReplaceParamInExpression err pvalue.isValid", !matchedKey.IsEmpty () ? matchedKey : partc_normal);
                    } else {
        ...
        for (const auto& poss_values : possibleEnumValues) {
            switch (property.definition.valueType) {
                case API_PropertyIntegerValueType:
                    ...
                    break;
                case API_PropertyRealValueType:
                    if (!is_equal (property.value.singleVariant.variant.doubleValue, poss_values.displayVariant.doubleValue)) {
                        guidValue = poss_values.keyVariant.guidValue;
                    }
                    break;
                case API_PropertyBooleanValueType:
...
bool ParamHelpers::Write (const API_Guid& elemGuid, ParamDictValue& params)
{
    ...
    for (const short inx : paramTypesListWrite) {
        ...
        if (paramByTypePtr == nullptr) continue;
...
void ParamHelpers::WriteInfo (ParamDictElement& paramToWrite)
{
    ...
    for (auto elemIt = paramToWrite.EnumeratePairs (); elemIt != nullptr; ++elemIt) {
        ...
        for (auto paramIt = params.EnumeratePairs (); paramIt != nullptr; ++paramIt) {
            ...
            const ParamValue& param = *paramIt->value;
            #endif
            if (!param.fromInfo) continue;
            const GS::UniString& paramName = param.name;
            if (paramsinfo.ContainsKey (paramName)) continue;
...
void ParamHelpers::WriteGDL (const API_Guid& elemGuid, ParamDictValue& params)
{
    ...
    for (Int32 i = 0; i < addParNum; ++i) {
        ...
        if (actualParam.typeMod != API_ParSimple) continue; //TODO Добавить замись массивов
        ...
        switch (actualParam.typeID) {
            ...
            case APIParT_CString:
                ...
                GS::ucscpy (strValuePtr, pValuePtr->val.uniStringValue.ToUStr (0, GS::Min (pValuePtr->val.uniStringValue.GetLength (), (USize) MaxStrValueLength)).Get ());
            ...
            case APIParT_PenCol:
                if (pValuePtr->val.intValue > 0 && pValuePtr->val.intValue < 255) chgParam.realValue = pValuePtr->val.intValue;
                break;
        ...
        if (type != API_ZombieAttrID) {
            ...
            if (attribinxint > 0) chgParam.realValue = attribinxint;
        }
        #if defined(AC_27) || defined(AC_28) || defined(AC_29)
        ...
        if (err != NoError) {
            ...
            err = ACAPI_Goodies (APIAny_CloseParametersID);
            #endif
            if (err != NoError) msg_rep ("ParamHelpers::WriteGDL", "APIAny_CloseParametersID", err, elem_head.guid);
            return;
        }
    }
    ...
    #endif
    if (err_ != NoError) msg_rep ("ParamHelpers::WriteGDL", "APIAny_CloseParametersID", err, elem_head.guid);
...
void ParamHelpers::WriteProperty (const API_Guid& elemGuid, ParamDictValue& params)
{
    ...
    GS::Array<API_Property> property2write; property2write.SetCapacity (params.GetSize ());
    for (ParamDictValue::PairIterator cIt = params.EnumeratePairs (); cIt != NULL; ++cIt) {
        #if defined(AC_28) || defined(AC_29)
        ParamValue& param = cIt->value;
        #else
...
void ParamHelpers::Read (const API_Guid& elemGuid, ParamDictValue& params, ParamDictComposite& paramcomposite, ListData::LibElement& paramListData, bool needReturnComposite, bool needListData)
{
    ...
    for (ParamDictValue::PairIterator cIt = params.EnumeratePairs (); cIt != NULL; ++cIt) {
        ...
        if (param.fromGDLdescription && can_read_fromGDL) needGetElement = true;
...
// --------------------------------------------------------------------
bool ParamHelpers::SubGuid_GetParamValue (const API_Guid& elemGuid, const GS::Array<API_PropertyDefinition>& definitions, ParamDictValue& subproperty)
{
    if (definitions.IsEmpty ()) return false;
    ...
    for (const auto& property : properties) {
        ...
        if (elemGuid != APINULLGuid) pvalue.fromGuid = elemGuid;
        if (!subproperty.ContainsKey (pvalue.rawName)) subproperty.Put (pvalue.rawName, std::move (pvalue));
        flag_add = true;
    }
    #if defined(TESTING)
    if (!flag_add) {
...
void ParamHelpers::CompareParamDictElement (ParamDictElement& paramsFrom, ParamDictElement& paramsTo)
{
    ...
    for (auto& cIt : paramsTo) {
        ...
        API_Guid elemGuid = *cIt.key;
        #endif
        if (ParamDictValue* pfromPtr = paramsFrom.GetPtr (elemGuid)) ParamHelpers::CompareParamDictValue (*pfromPtr, paramTo);
    }
}
...
// --------------------------------------------------------------------
bool ParamHelpers::ReadProperty (const API_Guid& elemGuid, ParamDictValue& params, const GS::Array<API_PropertyDefinition>& propertyDefinitions)
{
    if (params.IsEmpty ()) return false;
    #if defined(TESTING)
    DBprnt ("      ReadProperty");
...
bool ParamHelpers::ReadClassification (const API_Guid& elemGuid, ParamDictValue& paramByType)
{
    ...
    #endif
    if (!ClassificationFunc::ReadSystemDict ()) return false;
...
bool ParamHelpers::ReadGDL (const API_Element& element, const API_Elem_Head& elem_head, ParamDictValue& params, ParamDictValue& allparams)
{
    ...
    for (ParamDictValue::PairIterator cIt = params.EnumeratePairs (); cIt != NULL; ++cIt) {
        ...
        if (param.fromGDLArray) {
            ...
            if (param.fromGDLdescription && eltype == API_ObjectID) {
                ...
                if (param.fromGDLparam) paramByName.Put (rawName, param);
            }
        }
        if (param.fromGDLdescription && eltype == API_ObjectID) {
            paramBydescription.Put (param.rawName, param);
        } else {
            if (param.fromGDLparam) paramByName.Put (param.rawName, param);
        }
    }
    ...
    if (flag_find_desc && flag_find_name) {
        for (ParamDictValue::PairIterator cIt = paramBydescription.EnumeratePairs (); cIt != NULL; ++cIt) {
            ...
            if (paramByNamePtr == nullptr) continue;
    ...
    if (!flag_find_name) return flag_find_name;
    for (const auto& cIt : paramByName) {
        ...
        #endif
        if (!p.isValid) {
            if (element.header.guid == elem_head.guid) AddUnreadGDLParams (element.object.libInd, k);
            continue;
        }
...
bool ParamHelpers::GDLParamByName (const API_Element& element, const API_Elem_Head& elem_head, ParamDictValue& params, GS::HashTable<GS::UniString, GS::Array<GS::UniString>>& paramnamearray)
{
    ...
    auto UpdateParamValue = [&](ParamValue* pvaluePtr, const API_AddParType& actualParam) {
        if (pvaluePtr == nullptr) return false;
...
void ParamHelpers::ReadMaterial_ReadAddParam (ParamDictValue& paramsAdd, ParamDictComposite& paramcomposite, ParamDictValue& params, bool needReadQuantities)
{
    ...
    for (ParamDictComposite::PairIterator cIt = paramcomposite.EnumeratePairs (); cIt != NULL; ++cIt) {
        ...
        if (!param_composite.templatestring.Contains (BRACESTART)) continue;
        bool flag = false;
...
bool ParamHelpers::ReadMaterial (const API_Element & element, ParamDictValue & params, ParamDictComposite & paramcomposite, bool& needReadQuantities)
{
    ...
    if (!ParamHelpers::Components (element, params, paramsAdd, paramcomposite, needReadQuantities)) return false;
    ...
    for (ParamDictComposite::PairIterator cIt = paramcomposite.EnumeratePairs (); cIt != NULL; ++cIt) {
        ...
        if (param_composite.templatestring.Contains (BRACESTART)) {
            ...
            for (Int32 i = 0; i < nlayers; ++i) {
                ...
                if (param_composite.hasFormula) {
                    if (!stringformat.IsEmpty ()) templatestring.ReplaceAll (STRFORMULAEND + stringformat, STRFORMULAEND);
                ...
                if (layer_thickness_ptr != nullptr) {
                    ...
                    if (layer_thickness_ptr->val.formatstring.isEmpty) {
                        layer_thickness_ptr->val.formatstring = FormatStringFunc::ParseFormatString (DEFULTLEGHTFSTRING);
                    }
                ...
                if (layer_minthickness_ptr != nullptr) {
                    if (!is_equal (param_composite.composite[indx].fillThick_min, -1.0) && !is_equal (param_composite.composite[indx].fillThick_min, param_composite.composite[indx].fillThick)) {
                        ...
                        if (layer_minthickness_ptr->val.formatstring.isEmpty) {
                            layer_minthickness_ptr->val.formatstring = FormatStringFunc::ParseFormatString (DEFULTLEGHTFSTRING);
                        }
        ...
        if (flag) {
            if (param_composite.hasFormula) {
                if (outstring.Contains (BRACESTART)) ParamHelpers::ReplaceParamInExpression (params, outstring);
                if (!stringformat.IsEmpty ()) {
...
// -----------------------------------------------------------------------------
bool ParamHelpers::ConvertToParamValue (ParamValueData & pvalue, const API_AddParID & typeIDr, const GS::UniString & pstring, double preal)
{
...
bool ParamHelpers::ConvertBoolToParamValue (ParamValue & pvalue, const GS::UniString & paramName, const bool boolValue)
{
    ...
    pvalue.val.canCalculate = true;
    pvalue.isValid = true;
    if (pvalue.val.formatstring.isEmpty) pvalue.val.formatstring = FormatStringFunc::ParseFormatString (DEFULTINTFSTRING);
    return true;
}
...
bool ParamHelpers::ConvertToParamValue (ParamValue & pvalue, const API_IFCProperty & property)
{
    if (pvalue.rawName.IsEmpty () || pvalue.name.IsEmpty ()) {
        ...
        if (pvalue.rawName.IsEmpty ()) pvalue.rawName = IFCNAMEPREFIX + fname.ToLowerCase () + BRACEEND;
        if (pvalue.name.IsEmpty ()) pvalue.name = fname;
    }
    pvalue.isValid = true;
...
// --------------------------------------------------------------------
bool ParamHelpers::ComponentsCompositeStructure (const API_Guid & elemguid, API_AttributeIndex & constrinx, ParamDictValue & params, ParamDictComposite & paramcomposite, ParamDictValue & paramsAdd, double& width, double& length)
{
    #if defined(TESTING)
...
bool ParamHelpers::ComponentsProfileStructure (ProfileVectorImage & profileDescription, ParamDictValue & params, ParamDictComposite & paramcomposite, ParamDictValue & paramsAdd, double& width, double& length, bool& needReadQuantities)
{
    #if !defined(AC_22) && !defined(AC_23)
    #if defined(TESTING)
...
// --------------------------------------------------------------------
bool ParamHelpers::Components (const API_Element & element, ParamDictValue & params, ParamDictValue & paramsAdd, ParamDictComposite & paramcomposite, bool& needReadQuantities)
{
    #if defined(TESTING)
    DBprnt ("        Components");
    #endif
    ...
    if (structtype == API_CompositeStructure) hasData = ParamHelpers::ComponentsCompositeStructure (elemhead.guid, constrinx, params, paramcomposite, paramsAdd, width, length);
    #ifndef AC_23
    if (structtype == API_ProfileStructure) {
        ...
        hasData = ParamHelpers::ComponentsProfileStructure (profileDescription, params, paramcomposite, paramsAdd, width, length, needReadQuantities);
        ACAPI_DisposeElemMemoHdls (&memo);
    }
    #endif
    return hasData;
...
bool ParamHelpers::GetAttributeValues (const API_AttributeIndex & constrinx, ParamDictValue & params, ParamDictValue & paramsAdd)
{
    ...
    struct PendingAdd { API_Guid guid; ParamValue value; };
    ...
    auto getAttribute = [](API_AttrTypeID typeID, const API_AttributeIndex& index, GS::UniString& outName, API_Attribute& outAttrib) -> bool {
        outAttrib = {};
    ...
    auto ensureClassificationsRead = [&]() -> bool {
        if (isClassRead) return isClassReadOk;
        isClassRead = true;
    ...
    for (ParamDictValue::PairIterator cIt = params.EnumeratePairs (); cIt != NULL; ++cIt) {
        ...
        if (param.rawName.Contains (CharENTER)) {
            #if defined(TESTING)
            if (!param.rawName.Contains ("sync_name")) DBprnt ("GetAttributeValues err - param.rawName.Contains (CharENTER)", param.rawName);
            #endif
            continue;
        } else if (param.definition.name.Contains (CharENTER)) {
            #if defined(TESTING)
            if (!param.rawName.Contains ("sync_name")) DBprnt ("GetAttributeValues err - param.definition.name.Contains (CharENTER)", param.rawName);
            #endif
            continue;
        }
        ...
        if (const auto* p = params.GetPtr (rawName)) {
            if (p->isValid) {
                ...
                #if defined(TESTING)
                if (!param.rawName.Contains ("sync_name")) DBprnt ("GetAttributeValues err - !p->isValid", rawName);
                #endif
            }
        }
